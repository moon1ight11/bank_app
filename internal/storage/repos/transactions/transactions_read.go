...
func (db *Repo) GetAllUsersTransactions(userID uuid.UUID) ([]Transaction, error) {
	query := `
...
func (db *Repo) GetTransactionsByAccount(userID uuid.UUID, accountID uuid.UUID) ([]Transaction, error) {
	query := `
				SELECT id, user_from, account_from, user_to, account_to, amount, currency, timestamp
				FROM bank_app.transactions
...
func (db *Repo) GetTransactionByID(transactionID uuid.UUID, userID uuid.UUID) (Transaction, error) {
	query := `
				SELECT id, user_from, account_from, user_to, account_to, amount, currency, timestamp
				FROM bank_app.transactions
	...
	if err != nil {
		return Transaction{}, fmt.Errorf("error in GetTransactionByID query: %w", err)
	}
